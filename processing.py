...
for article in splitArticles:
    ...
    for item in valCount.index:
        if item not in termDict.values() and item not in stopwords:
            print(item)
            termDict[termInd] = item
            termInd += 1
    index += 1
#print(termDict.values())
#for matRow in range(0,index):
    #for matCol in range(0,termInd):
        #word = termDict[matCol]
